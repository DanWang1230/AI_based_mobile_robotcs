...
def predict_gp(train_x, train_y, init_state, action_traj):
    ...
    CovMatrix = np.zeros((4, M, M))

    # store the covariance matrix for GP
    for k in range(4):
        for i in range(M):
            for j in range(M):
                CovMatrix[k][i][j] = kernel_scale_factors[k]**2 * \
                    np.exp(- np.dot(np.dot(train_x[i]-train_x[j], np.diag(1/kernel_length_scales[:,k]**2)), train_x[i]-train_x[j]) /2)

    for t in range(H):
        test_x = augmented_state(x, action_traj[t]) # 1*6

        for k in range(4):
            kstar = np.array([kernel_scale_factors[k]**2 * \
                np.exp(- np.dot(np.dot(test_x-train_x[i], np.diag(1/kernel_length_scales[:,k])**2), test_x-train_x[i]) /2)\
                    for i in range(M)])
            
            temp = np.linalg.inv(CovMatrix[k] + np.identity(M) * noise_sigmas[k]**2)
        
            pred_gp_mean[t][k] = np.dot(np.dot(kstar, temp) , train_y[:,k])
            pred_gp_variance[t][k] = kernel_scale_factors[k]**2 - np.dot(np.dot(kstar, temp) , kstar) # variance, not standard deviation
    ...
    for j in range(N):
        x_sampled[j] = np.copy(init_state)
        for t in range(H):
            test_x_sampled = augmented_state(x_sampled[j], action_traj[t])
            for k in range(4):
                kstar_sampled = np.array([kernel_scale_factors[k]**2 * \
                    np.exp(- np.dot(np.dot(test_x_sampled-train_x[i], np.diag(1/kernel_length_scales[:,k])**2), test_x_sampled-train_x[i]) /2)\
                        for i in range(M)])
                
                temp_sampled = np.linalg.inv(CovMatrix[k] + np.identity(M) * noise_sigmas[k]**2)
                pred_gp_mean_trajs[j][t][k] = np.dot(np.dot(kstar_sampled, temp_sampled) , train_y[:,k])
                pred_gp_variance_trajs[j][t][k] = kernel_scale_factors[k]**2 - np.dot(np.dot(kstar_sampled, temp_sampled) , kstar_sampled) # variance, not standard deviation
                
            samp = rng.normal(pred_gp_mean_trajs[j][t], np.sqrt(pred_gp_variance_trajs[j][t]))  
            x_sampled[j] += samp
            rollout_gp_trajs[j][t] = x_sampled[j]
