...
def setup_parser():
    ...
    parser.add_argument(
        '--num-particles', type=int, default=500,
...
if __name__ == '__main__':
    ...
    with open('MPE_pf_d_500.csv', mode='w') as meanPE:
        MPE_writer = csv.writer(meanPE, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        for i in range(6):
            MPE_writer.writerow(mean_position_error_all[i,:])
    with open('ANEES_pf_d_500.csv', mode='w') as Anees:
        ANEES_writer = csv.writer(Anees, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        for i in range(6):
            ANEES_writer.writerow(anees_all[i,:])
