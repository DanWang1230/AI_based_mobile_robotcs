...
class MapEnvironment(object):
    ...
    def state_validity_checker(self, config):
        ...
        # TODO: YOUR IMPLEMENTATION HERE
        n = np.shape(config)[1]
        nrow = np.shape(self.map)[0]
        ncol = np.shape(self.map)[1]
        for i in range(n):
            if 0 <= config[0,i] < nrow and 0 <=config[1,i] < ncol:
                if self.map[int(config[0,i]), int(config[1,i])] == 1:
                    return False
        return True
    ...
    def h(self, config):
        ...
        # TODO: YOUR IMPLEMENTATION HERE
        # Euclidean distance
        return np.linalg.norm(config - self.goal)
